...
func download(loc, moe, step):
	...
	if OS.get_name() == "Windows":
		...
		if step == 0:
			Progress.set_visible(true)
			if moe == false:
				ProgressLabel.text = "Downloading SRB2Kart..."
				Progress.value = 2
				_request.set_download_file(dir.get_current_dir()+"/"+"srb2kart-x86_64.AppImage")
				_request.request("https://github.com/Lonsfor/srb2k-appimage/releases/download/latest/srb2kart-x86_64.AppImage")
			elif moe == true:
				ProgressLabel.text = "Downloading SRB2Kar-Moe..."
				Progress.value = 2
				_request.set_download_file(dir.get_current_dir()+"/"+"srb2kart-moe-x86_64.AppImage")
				_request.request("https://github.com/Lonsfor/srb2k-appimage/releases/download/latest/srb2kart-moe-x86_64.AppImage")
		else:
			ProgressLabel.text = "Setting permission"
			Progress.value = 3
			yield(get_tree().create_timer(1.0), "timeout")
			# give it executable permissions
			OS.execute("chmod", PoolStringArray(["+x", _request.download_file]), true)
